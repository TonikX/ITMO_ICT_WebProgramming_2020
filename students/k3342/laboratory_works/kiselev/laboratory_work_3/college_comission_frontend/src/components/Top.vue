...
export default {
  ...
  mounted () {
    this.axios
      .get('http://127.0.0.1:8000/api/applications/all?top=1')
  ...
  methods: {
    ...
    submit () {
      ...
      this.axios
        .get(`http://127.0.0.1:8000/api/applications/all?top=1&from_date=${this.from_date}&to_date=${this.to_date}&class=${class_type}&spec=${spec}&privelege=${privelege}&state=${state}&min=${this.minimal_point}`)
        .then(response => { this.showApplications(response.data) })
    },
    clear () {
      this.from_date = ''
