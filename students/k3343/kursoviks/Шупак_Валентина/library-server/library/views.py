...
class Query1(APIView):
    ...
    def get(self, request):
        ...
        book_count = TakingBook.objects.filter(reader=reader).count()
        i = 0
        book_name = []
        while i != book_count:
            book = TakingBook.objects.filter(reader=reader)[i].book
            book_name.append(Book.objects.filter(name=book)[0].name)
            i += 1
        print(book_name)
        return Response({'result': book_name})
