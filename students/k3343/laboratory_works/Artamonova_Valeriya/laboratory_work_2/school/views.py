...
class Query5(APIView):
    ...
    def get(self, request):
        base = Cabinet.objects.filter(profile='Для_профильных_дисциплин').count()
        profile = Subject.objects.filter(profile='Для_базовых_дисциплин').count()
        results = {'Кабинеты для профильных дисциплин': profile, 'Кабинеты для базовых дисциплин': base}
        return Response({'data': results})
