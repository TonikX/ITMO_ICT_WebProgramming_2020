...
export default {
  ...
  mounted () {
  	this.axios
  	  .get(`http://${window.location.hostname}:8000/api/applications/?exam=1`)
	  .then(response => { this.enrolleesData = response; this.getEnrollee() })
	  .catch(err => { console.error('API', err) })

  	this.axios
	  .get(`http://${window.location.hostname}:8000/api/specializations/`)
  ...
  methods: {
  	...
  	filterEnrollee () {
  	  ...
  	  let url = `http://${window.location.hostname}:8000/api/applications/?exam=1&specialization=${specialization}&education_form=${educationForm}`
  	...
  	clear () {
  	  ...
  	  this.axios
  	    .get(`http://${window.location.hostname}:8000/api/applications/`)
	    .then(response => { this.enrolleesData = response; this.getEnrollee() })
	    .catch(err => { console.error('API', err) })
  	}
  }
