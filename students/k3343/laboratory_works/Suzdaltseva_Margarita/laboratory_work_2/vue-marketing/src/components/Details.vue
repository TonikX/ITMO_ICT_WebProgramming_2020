...
export default {
  ...
  methods: {
    ...
    getPayment (data) {
      let pay_requested = new Date(Date.parse(data[0].day_requested))
      let pay_paid = data[0].day_paid ? new Date(Date.parse(data[0].day_paid)) : ''
      this.payment = data[0].id
      this.pay_amount = data[0].amount
      this.pay_requested = pay_requested
